...
const pollRunsLogic = async (typeFilter = null) => {
  ...
  for (const doc of snapshot.docs) {
    const run = doc.data();
    if (typeFilter === 'posts' && run?.type === 'stories') continue; 
    if (typeFilter === 'stories' && run?.type !== 'stories') continue; 
    ...
    try {
      ...
      if (status === 'SUCCEEDED') {
        ...
        if (dsId) {
          ...
          // TRIGGER UNIFIED SINGLE-PASS PROCESSING
          try { 
            await unifiedProcessRunInternal(runId, { model: 'gpt-5.2' }); 
          } catch (e) {
            logger.error('Unified process trigger failed', { runId, error: e.message });
          }
          continue;
        }
      }
      ...
      logger.error('pollRunsLogic error', { runId: runId || doc.id, error: error.message });
    }
  }
